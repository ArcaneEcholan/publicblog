...
refresh()
  ...
  │     ├── 实例化 bean (构造), 此时执行 ConfigurationPropertiesBindingPostProcessor，绑定 @ConfigurationProperties
  │     ├── BeanPostProcessor.before 此时会执行 AutowiredAnnotationBeanPostProcessor, 注入 @Value / @Autowired
...
```text
├── 创建 Environment 发布 ApplicationEnvironmentPreparedEvent, 在这里监听 Environment，可修改 PropertySource
├── refresh()
      ├── ... BeanFactoryPostProcessor ...
      ├── registerBeanPostProcessors()
      ...
      ├── registerListeners()
      ├── finishBeanFactoryInitialization()
      │     ├── Bean 构造 + 注入 + 此时执行 ConfigurationPropertiesBindingPostProcessor，绑定 @ConfigurationProperties
      │     ├── BeanPostProcessor.before 此时会执行 AutowiredAnnotationBeanPostProcessor, 注入 @Value / @Autowired
      │     ├── @PostConstruct / afterPropertiesSet / init-method
      │     ├── BeanPostProcessor.after
      │     └── ...
