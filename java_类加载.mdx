...
而且一旦 static {} 块儿中初始化失败，这个类就废了，后续再用这个类就会报 NoClassDefFoundError。
...
class Test {
    public static void main(String[] args) {
        try {
            ...
            System.out.println(Test1.i); // 这里报错 ExceptionInInitializerError
        } catch (Throwable e) {
            Throwable root = Utils.getRootCause(e);
            System.out.println(root.getClass().getName() + ": " + root.getMessage());
        }

        try {
            System.out.println(2);
            System.out.println(Test1.i); // 这里直接报错 NoClassDefFoundError，说明 static {} 报错直接导致类加载失败。
        } catch (Throwable e) {
            Throwable root = Utils.getRootCause(e);
            System.out.println(root.getClass().getName() + ": " + root.getMessage());
        }
...
2
java.lang.NoClassDefFoundError: Could not initialize class Test1
```
...
* 创建LaunchedURLClassLoader(这个类加载器重写了上述 findclass -> defineclass -> resolveclass 这几个方法，是自定义类加载的实践典范，可以参考。)
